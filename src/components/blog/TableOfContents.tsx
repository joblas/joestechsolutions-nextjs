      ...
      <ol className="space-y-1.5 list-none p-0 m-0">
        {items.map((item, i) => (
          <li key={item.id}>
            <a
              href={`#${item.id}`}
              className="flex items-center gap-2 text-white/50 hover:text-[#0EA5E9] text-sm transition-colors group"
            >
              <span className="text-white/20 group-hover:text-[#0EA5E9]/50 text-xs tabular-nums w-5">{String(i + 1).padStart(2, "0")}</span>
              {item.label}
            </a>
          </li>
        ))}
