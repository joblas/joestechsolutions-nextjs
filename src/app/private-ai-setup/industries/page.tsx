          ...
          <div className="text-center space-y-8 max-w-4xl mx-auto">
            <FadeIn delay={0.1}>
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#0EA5E9]/10 border border-[#0EA5E9]/20 mb-6">
                ...
                <span className="text-[#0EA5E9] font-medium">9 Industry Assistants</span>
              </div>
            </FadeIn>
            <FadeIn delay={0.15}>
                ...
                <StaggerItem key={vertical.slug}>
                  <AnimatedCard>
                    <Link href={`/private-ai-setup/industries/${vertical.slug}`} className="block h-full">
                      <Card className="bg-[#1c1c26] border-white/10 hover:border-[#0EA5E9]/50 transition-all duration-500 h-full group cursor-pointer overflow-hidden relative">
                        <div className="absolute top-0 right-0 w-48 h-48 bg-linear-to-br from-[#0EA5E9]/5 to-transparent rounded-full blur-3xl" />
                        <CardContent className="p-8 relative">
                          <div className="flex items-start justify-between mb-6">
                            <div className="w-14 h-14 bg-[#0EA5E9]/10 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform">
                              {IconComponent && <IconComponent weight="duotone" className="h-7 w-7 text-[#0EA5E9]" />}
                            </div>
                            {vertical.isPremium && (
                          ...
                          <h2 className="text-2xl font-bold text-white font-space-grotesk mb-2 group-hover:text-[#0EA5E9] transition-colors">
                            {vertical.name}
                          </h2>
                          <p className="text-[#0EA5E9] text-sm font-medium mb-3">{vertical.tagline}</p>
                          <p className="text-white/60 text-sm leading-relaxed mb-6 line-clamp-3">
                            {vertical.description}
                          </p>

                          <div className="flex items-center gap-2 text-[#0EA5E9] font-medium text-sm group-hover:gap-3 transition-all">
                            Learn More
